...
class ReporteCompras(models.AbstractModel):
    _name = 'report.l10n_hn_extra.reporte_compras'
    ...
    def lineas(self, datos):
        ...
        journal_ids = [x for x in datos['diario_ids']]
        filtro = [
            ('state','in',['posted']),
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if len(data['form']['diario_ids']) == 0:
            raise UserError("Por favor ingrese al menos un diario.")

        diario = self.env['account.journal'].browse(data['form']['diario_ids'][0])
